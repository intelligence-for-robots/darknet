...
class yoloRotatebbox:
    ...
    def __init__(self, filename, image_ext, angle, dir_path):
        print(dir_path + filename + image_ext)
        print(dir_path + filename + ".txt")
        assert os.path.isfile(dir_path + filename + image_ext)
        assert os.path.isfile(dir_path + filename + ".txt")
        ...
        self.image = cv2.imread(self.dir_path + self.filename + self.image_ext, 1)
    ...
    def rotateYolobbox(self):
        ...
        f = open(self.dir_path + self.filename + '.txt', 'r')
        f1 = f.readlines()
        new_bbox = []
...
if __name__ == "__main__":
    ...
    dir_path="/home/ubuntu/data/SewerData-ROI_v5/data/obj/"
    ...
    for filename in tqdm(os.listdir(dir_path)):
        ...
        if(file[-1]=="jpg"):
            ...
            continue
        for angle in angels:
            im = yoloRotatebbox(image_name, image_ext, angle, dir_path)
            bbox = im.rotateYolobbox()
            image = im.rotate_image()
            # to write rotateed image to disk
            cv2.imwrite(dir_path + image_name+'_' + str(angle) + '.jpg', image)
            file_name = dir_path + image_name+'_' + str(angle) + '.txt'
